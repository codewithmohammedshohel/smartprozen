...
if (empty($token)) {
    ...
    $stmt = $conn->prepare("SELECT email, token as hashed_token, created_at FROM password_resets WHERE token = ?");
    $stmt->bind_param("s", $token);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($reset_request = $result->fetch_assoc()) {
        // Verify the token (plain-text from URL vs. hashed from DB)
        if (password_verify($token, $reset_request['hashed_token'])) {
            // Check expiry (e.g., 1 hour)
            $expiry_time = strtotime($reset_request['created_at']) + (3600); // 1 hour expiry
            if (time() < $expiry_time) {
            ...
            $error_message = 'Invalid password reset token.'; // Token mismatch
        }
    } else {
        $error_message = 'Invalid password reset token.'; // Token not found
    }
}?>
